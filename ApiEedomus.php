...
class  ApiEedomus {
    ...
    public function getHistoriquePeripherique($idPeripherique, $start_date, $end_date, $show_all=0){
        if(!$idPeripherique){
        ...
        $json = file_get_contents($this->api_protocole.'://'.$this->api_url.'/get?action=periph.history&periph_id='.$idPeripherique.'&start_date='.$start_date.'&end_date='.$end_date.'&api_user='.$this->api_user.'&api_secret='.$this->api_secret.'&show_all='.$show_all);
